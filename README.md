# Collection of Remote Sensing datasets for use in Deep Learning applications

## Land Use 
- [EuroSAT: Land Use and Land Cover Classification with Sentinel-2 (2018)](https://github.com/phelber/EuroSAT)

- [BigEarthNet: A Large-Scale Sentinel Benchmark Archive - Multilabeled Sentinel 1 and Sentinel 2 datasets with CORINE classes (2019)](https://bigearth.net/)

- [OPTIMAL31 - Scene classification dataset (2018)](https://onedrive.live.com/?authkey=%21AN26tA9MG5tcwg0&cid=5479ABC21673380B&id=5479ABC21673380B%2146678&parId=root&action=locate)
    - [OPTIMAL31 Paper](https://ieeexplore.ieee.org/document/8454883)

- [SAT4 & SAT6 (2015)](http://csc.lsu.edu/~saikat/deepsat/)
    - [SAT4 & SAT6 paper](https://dl.acm.org/doi/10.1145/2820783.2820816)

- [UC Merced Land Use Dataset (2010)](http://weegee.vision.ucmerced.edu/datasets/landuse.html)
    -[UC Merced Paper](https://dl.acm.org/doi/10.1145/1869790.1869829)

## Object Detection
- [NWPU VHR-10 (ten-class geospatial object detection data set) & SSDD: SAR ship detection dataset](https://github.com/chaozhong2010/VHR-10_dataset_coco)

- [Defence Science and Technology Laboratory (Dstl) Worldvie 3 10 classes (2016)](https://www.kaggle.com/competitions/dstl-satellite-imagery-feature-detection/data)

- [iSAID: A Large-scale Dataset for Instance Segmentation in Aerial Images (2019)](https://captain-whu.github.io/iSAID/)
    - [iSAID paper](https://arxiv.org/abs/1905.12886)

- [DOTA: A Large-Scale Benchmark and Challenges for Object Detection in Aerial Images (2018)](https://captain-whu.github.io/DOTA/dataset.html)
    - [DOTA paper](https://ieeexplore.ieee.org/document/8578516)

- [MASATI dataset (v2) - MAritime SATellite Imagery dataset. Ship detection. (2019)](https://www.iuii.ua.es/datasets/masati/)

## Various
- [Spacenet (Different datasets through AWS)](https://spacenet.ai/datasets/)

## Focused on flooding
- [FloodNet: UAV pre- and post-flooding imagery (2021)](https://github.com/BinaLab/FloodNet-Supervised_v1.0)
    - [FloodNet paper](https://ieeexplore.ieee.org/document/9460988)

Spacenet 8 challenge is also about flood detection ([link](https://spacenet.ai/sn8-challenge/)) 

## Forests and Trees
- [Wilt: diseased pine and oak trees classification (uses Quickbird) (2013)](https://archive.ics.uci.edu/ml/datasets/wilt#)
    - [Willt paper](https://www.tandfonline.com/doi/abs/10.1080/01431161.2013.810825)

- [Forest type mapping Data Set (2015)](https://archive.ics.uci.edu/ml/datasets/Forest+type+mapping#)

